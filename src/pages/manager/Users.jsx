...
export default function UserManagement() {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(user => {
      ...
      const matchesInstitution = institutionFilter === 'all' || user.institutionType === institutionFilter;
  ...
  return (
    ...
    >
      ...
      <SectionCard title="All Users" description="Complete list of platform users with their roles and institutions">
          ...
          <table className="w-full ">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-left py-3 px-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap w-[25%]">User</th>
                <th className="text-left py-3 px-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap w-[25%]">Contact</th>
                <th className="text-left py-3 px-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap w-[20%]">Role & Status</th>
                <th className="text-left py-3 px-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap w-[25%]">Institution</th>
                <th className="text-center py-3 px-4 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap w-[5%]">Actions</th>
              </tr>
            </thead>
                  ...
                  <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-slate-500 truncate">Joined: {user.joinDate}</p>
                          <p className="text-xs text-slate-500 truncate">Last active: {formatLastLogin(user.lastLogin)}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <div className="min-w-0">
                        <p className="text-sm text-slate-900 truncate max-w-50">{user.email}</p>
                        <p className="text-xs text-slate-500 truncate">{user.phone}</p>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex flex-col gap-1.5">
                        <div className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium whitespace-nowrap ${getRoleColor(user.role)}`}>
                          <AppIcon name="shield_check" size={12} />
                          {user.role}
                        </div>
                        <div className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium whitespace-nowrap ${
                          user.status === 'active' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                          'bg-amber-50 text-amber-700 border border-amber-200'
                        }`}>
                          <AppIcon name={user.status === 'active' ? 'check_circle' : 'pending'} size={12} />
                          {user.status}
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2 min-w-0">
                        <AppIcon name={getInstitutionIcon(user.institutionType)} size={14} className="text-slate-600 shrink-0" />
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-slate-900 truncate max-w-45">{user.institution}</p>
                          <p className="text-xs text-slate-500 capitalize truncate">{user.institutionType}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
